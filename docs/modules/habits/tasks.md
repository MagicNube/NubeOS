...
**Resultado provisional:** los tipos Rust validan identificador, nombre, tipo, categoría, icono y las cuatro frecuencias mediante conjuntos cerrados.
...
**Resultado provisional:** el evaluador usa lunes como inicio de semana, meses naturales, días estrictos, objetivos flexibles, omisiones y pausas históricas.
...
**Resultado provisional:** la migración 0010 crea cinco tablas propias con restricciones, índices y cascadas acotadas; la reapertura SQLite está probada.
...
**Resultado provisional:** crear y editar conserva revisiones fechadas; una prueba confirma que cada fecha resuelve la configuración que le corresponde.
...
**Resultado provisional:** completar, omitir y desmarcar usan una única fila por actividad y fecha; Rust rechaza fechas futuras o no aplicables.
...
**Resultado provisional:** pausa y archivo abren intervalos neutrales; restaurar respeta una pausa previa y el borrado definitivo exige Archivo.
...
**Resultado provisional:** Rust entrega celdas, objetivos nominales y efectivos, porcentajes, rachas, última realización y próxima fecha sin cálculos de negocio en React.
...
**Resultado provisional:** once comandos pequeños cubren catálogo, registros, ciclo de vida, orden, vistas y estadísticas con contratos `camelCase`.
...
**Resultado provisional:** Hoy, Mañana, Semana y Mes se cargan de forma diferida, refrescan sin sustituir la vista por un cargador y cambian de día con `Europe/Madrid`.
...
**Resultado provisional:** el catálogo incluye búsqueda, filtros, modal compartido, iconos, pausa, Archivo y arrastre por puntero compatible con WebView2.
...
**Resultado provisional:** la vista compara porcentajes con muestra mínima y muestra rachas, realizaciones, progreso y última fecha por actividad.
...
- Verificación: `cargo fmt --check`, `cargo test`, `cargo clippy --all-targets -- -D warnings`, `pnpm build`, `git diff --check` y prueba manual pendiente de Nube.

**Resultado provisional:** esta primera revisión quedó superada con 70 pruebas Rust. La segunda iteración continúa en T-013 a T-019 y eleva la batería total a 75 pruebas.
...
**Resultado provisional:** `starts_on`, la tabla de días mensuales y la clave de icono extensible se migran sin perder los valores del esquema anterior. La fecha queda bloqueada después del primer registro.
...
**Resultado provisional:** Rust resuelve semana, mes, año, historial e intervalos personalizados, recorta el futuro y excluye el tiempo anterior al inicio de cada actividad.
...
**Resultado provisional:** el modal admite fecha inicial, excepciones semanales, varios hitos mensuales e iconos de mochila, planificación, tijeras, lavadora y cama.
...
**Resultado provisional:** Hoy y Mañana separan hábitos y tareas recurrentes en dos paneles y vuelven a una columna mediante CSS adaptable.
...
**Resultado provisional:** el checkbox conserva su comportamiento binario; `CircleMinus` gestiona la omisión y adopta un estado visual diferenciado. Los días orientativos usan una estrella únicamente cuando aporta información.
...
**Resultado provisional:** los selectores del catálogo comparten padding y geometría; Estadísticas ofrece los cinco periodos y conserva el resultado anterior durante la recarga.
...
**Resultado provisional:** las 75 pruebas Rust, Clippy estricto, el build TypeScript/Vite y `git diff --check` terminan correctamente. Falta el recorrido visual y funcional de Nube en Tauri.
...
**Resultado provisional:** las dos columnas muestran su propio progreso, las acciones ocupan posiciones estables y el editor es accesible desde cada actividad. El repositorio mantiene las frecuencias anteriores mediante revisiones fechadas.
...
**Resultado provisional:** el botón secundario tiene centrado explícito y el selector organiza veinte iconos en dos filas de diez. `icon_key` admite las nuevas opciones sin modificar datos existentes.
...
**Resultado provisional:** el progreso diario distingue explícitamente objetivos de esta semana y de este mes; Rust continúa siendo la autoridad de sus límites.

## T-023 — Cerrar la navegación y los periodos iniciales

- Estado: Implementada y aprobada
- Dependencias: T-002, T-009, T-022
- Alcance: retirar Mañana, sugerir el lunes en nuevas frecuencias semanales, identificar periodos iniciales parciales y avisar al editar semanas anteriores.
- Criterios: Hoy conserva el check rápido; Semana permite correcciones históricas con contexto; Rust calcula si el periodo es parcial; el futuro continúa bloqueado.
- Verificación: prueba Rust de una actividad iniciada en sábado, build TypeScript, Clippy estricto y recorrido funcional de Nube.

**Resultado:** la navegación final queda en Hoy, Semana, Mes, Estadísticas y Actividades. El primer periodo parcial es explícito y las semanas pasadas advierten del efecto de las correcciones.
