CREATE TABLE media_titles_v15 (
    ...
    score REAL CHECK (score IS NULL OR (score BETWEEN 0 AND 10 AND score * 10 = CAST(score * 10 AS INTEGER))),
    ...
    cover_mime_type TEXT CHECK (cover_mime_type IS NULL OR cover_mime_type IN ('image/jpeg', 'image/png', 'image/webp', 'image/gif')),
...
INSERT INTO media_titles_v15
SELECT * FROM media_titles;

CREATE TABLE media_contents_v15 (
    id TEXT PRIMARY KEY NOT NULL,
    title_id TEXT NOT NULL REFERENCES media_titles_v15(id) ON DELETE CASCADE,
    name TEXT NOT NULL CHECK (length(trim(name)) BETWEEN 1 AND 150),
...
INSERT INTO media_contents_v15
SELECT * FROM media_contents;

CREATE TABLE media_watch_sessions_v15 (
    id TEXT PRIMARY KEY NOT NULL,
    title_id TEXT REFERENCES media_titles_v15(id) ON DELETE CASCADE,
    content_id TEXT REFERENCES media_contents_v15(id) ON DELETE CASCADE,
...
-- Las sesiones antiguas conservan su delta como auditoría interna. Al no tener
-- episode_number no se muestran en el historial nuevo ni inventan fechas.
INSERT INTO media_watch_sessions_v15
    (id, title_id, content_id, watched_on, delta, source, created_at, episode_number)
SELECT id, title_id, content_id, watched_on, delta, source, created_at, NULL
...
ALTER TABLE media_titles_v15 RENAME TO media_titles;
ALTER TABLE media_contents_v15 RENAME TO media_contents;
ALTER TABLE media_watch_sessions_v15 RENAME TO media_watch_sessions;
...
CREATE INDEX idx_media_contents_studio
ON media_contents(studio COLLATE NOCASE);
...
CREATE UNIQUE INDEX media_sessions_unique_content_episode
ON media_watch_sessions(content_id, episode_number)
WHERE content_id IS NOT NULL AND episode_number IS NOT NULL;
CREATE UNIQUE INDEX media_sessions_unique_title_unit
ON media_watch_sessions(title_id, episode_number)
WHERE title_id IS NOT NULL AND episode_number IS NOT NULL;
