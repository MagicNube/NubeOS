import { Archive, ChevronDown, MoreHorizontal, Pencil, Plus, Search, X } from "lucide-react";
...
const initialValues: ProductFormValues = {
  name: "", category: "other", protein: "", carbohydrate: "", fat: "", kilocalories: "", supermarket: "",
  presentationKind: "package", packageTotalGrams: "", packagePriceEur: "", packageUnits: "", bulkWeightPriceEur: "", legacyPresentation: false,
};
...
function decimal(value: number) { return new Intl.NumberFormat("es-ES", { maximumFractionDigits: 1 }).format(value); }
...
function presentationSummary(presentation?: PurchasePresentation) {
  ...
  if (presentation.kind === "package") {
    ...
    if (presentation.unitsPerPackage) details.push(`${presentation.unitsPerPackage} uds`);
...
function SelectControl({ children }: { children: React.ReactNode }) {
  return <span className="select-control">{children}<ChevronDown aria-hidden="true" size={15} /></span>;
}

function NumberField({ label, value, onChange, required, min = "0", placeholder = "0", step = "1" }: { label: string; value: string; onChange: (value: string) => void; required?: boolean; min?: string; placeholder?: string; step?: string }) {
  return <label className="product-field"><span>{label}</span><input inputMode="decimal" min={min} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} required={required} step={step} type="number" value={value} /></label>;
}
...
function ProductForm({ product, onCancel, onSaved }: { product?: Product; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      if (product) await productApi.update(product.id, inputFromValues(values)); else await productApi.create(inputFromValues(values));
      await onSaved();
  ...
  return <form className="product-form" onSubmit={submit}>
    ...
    <div className="product-form-grid">
      ...
      <label className="product-field"><span>Categoría</span><SelectControl><select onChange={(event) => update("category", event.target.value as ProductCategory)} value={values.category}>{productCategories.map((category) => <option key={category} value={category}>{categoryLabels[category]}</option>)}</select></SelectControl></label>
      <label className="product-field"><span>Supermercado (opcional)</span><SelectControl><select onChange={(event) => update("supermarket", event.target.value as Supermarket | "")} value={values.supermarket}><option value="">Sin especificar</option>{Object.entries(supermarketLabels).map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></SelectControl></label>
    </div>
    <fieldset className="product-fieldset"><legend>Macros por 100 g</legend><div className="product-form-grid product-macro-grid"><NumberField label="Kcal" onChange={(value) => update("kilocalories", value)} required value={values.kilocalories} /><NumberField label="Proteínas (g)" onChange={(value) => update("protein", value)} required step="0.1" value={values.protein} /><NumberField label="Carbohidratos (g)" onChange={(value) => update("carbohydrate", value)} required step="0.1" value={values.carbohydrate} /><NumberField label="Grasas (g)" onChange={(value) => update("fat", value)} required step="0.1" value={values.fat} /></div></fieldset>
    <fieldset className="product-fieldset"><legend>Presentación de compra</legend><p className="product-fieldset-description">El nombre del producto será el que aparezca en tu lista de compra.</p>
      {values.legacyPresentation && <p className="legacy-note">Este producto usa datos heredados. Al guardarlo, elige una presentación actual.</p>}
      <label className="product-field"><span>Tipo</span><SelectControl><select onChange={(event) => update("presentationKind", event.target.value as PresentationKind)} value={values.presentationKind}><option value="package">Paquete, bolsa o bandeja</option><option value="bulkByWeight">A granel por peso</option></select></SelectControl></label>
      {values.presentationKind === "package" && <div className="product-form-grid product-presentation-fields"><NumberField label="Peso total (g)" min="1" onChange={(value) => update("packageTotalGrams", value)} required step="1" value={values.packageTotalGrams} /><NumberField label="Unidades por paquete (opcional)" min="1" onChange={(value) => update("packageUnits", value)} step="1" value={values.packageUnits} /><label className="product-field"><span>Precio del paquete (€) (opcional)</span><input inputMode="decimal" onChange={(event) => update("packagePriceEur", event.target.value)} placeholder="2,99" value={values.packagePriceEur} /></label></div>}
      {values.presentationKind === "bulkByWeight" && <div className="product-presentation-fields"><label className="product-field"><span>Precio por kg (€) (opcional)</span><input inputMode="decimal" onChange={(event) => update("bulkWeightPriceEur", event.target.value)} placeholder="2,99" value={values.bulkWeightPriceEur} /></label></div>}
    </fieldset>
...
export default function ProductsPage({ filters, onFiltersChange, onSearchMeals }: { filters: ProductCatalogFilters; onFiltersChange: (filters: ProductCatalogFilters) => void; onSearchMeals: (product: Product) => void }) {
  ...
  const [permanentDeletion, setPermanentDeletion] = useState<Product | null>(null);
  ...
  async function deletePermanently() { if (!permanentDeletion) return; try { await productApi.delete(permanentDeletion.id); setPermanentDeletion(null); await loadProducts(); } catch (reason) { setError(errorMessage(reason)); } }
  ...
  return <section className="products-page">
    <div className="products-toolbar"><div className="product-catalog-controls"><div className="catalog-search"><Search size={17} /><input aria-label="Buscar productos" onChange={(event) => onFiltersChange({ ...filters, query: event.target.value })} placeholder="Buscar productos" value={filters.query} /></div><div className="category-control"><button aria-expanded={categoryOpen} className="category-button" onClick={() => setCategoryOpen((value) => !value)} type="button"><span>{categoryLabel}</span><ChevronDown aria-hidden="true" size={15} /></button>{categoryOpen && <div className="category-menu"><button className={filters.category === "all" ? "active" : ""} onClick={() => { onFiltersChange({ ...filters, category: "all" }); setCategoryOpen(false); }} type="button">Todas las categorías</button>{productCategories.map((category) => <button className={filters.category === category ? "active" : ""} key={category} onClick={() => { onFiltersChange({ ...filters, category }); setCategoryOpen(false); }} type="button">{categoryLabels[category]}</button>)}</div>}</div></div><div className="toolbar-actions"><button className="archive-link" onClick={() => setStatus((current) => current === "active" ? "archived" : "active")} type="button"><Archive size={15} /> {status === "active" ? "Archivo" : "Volver al catálogo"}</button><button className="primary-button" onClick={() => setEditing(null)} type="button"><Plus size={16} /> Añadir producto</button></div></div>
    ...
    <div className="products-catalog-heading"><div><p className="section-kicker">{status === "active" ? "CATÁLOGO" : "ARCHIVO"}</p><h2>{status === "active" ? "Tus productos" : "Productos archivados"}</h2></div></div>
    ...
    {!loading && !error && visible.length > 0 && <div className="product-grid">{visible.map((product) => <article className="product-card" key={product.id}><div className="product-card-heading"><div><span className="product-category">{categoryLabels[product.category]}</span><h3>{product.name}</h3><p>{product.supermarket ? supermarketLabels[product.supermarket] : "Sin especificar"}</p></div><div className="card-icon-actions"><button aria-label={`Editar ${product.name}`} className="product-icon-button" onClick={() => setEditing(product)} type="button"><Pencil size={16} /></button><button aria-expanded={actionsFor === product.id} aria-label={`Más acciones de ${product.name}`} className="product-icon-button" onClick={() => setActionsFor((current) => current === product.id ? null : product.id)} type="button"><MoreHorizontal size={17} /></button>{actionsFor === product.id && <div className="card-menu"><button onClick={() => { setActionsFor(null); void changeStatus(product); }} type="button">{product.status === "active" ? "Archivar" : "Restaurar"}</button>{product.status === "active" && <><button onClick={() => { setActionsFor(null); void inspectRemoval(product); }} type="button">Retirar de recetas</button><button onClick={() => { setActionsFor(null); onSearchMeals(product); }} type="button">Buscar comidas con este producto</button></>}{product.status === "archived" && <button className="danger-menu-action" onClick={() => { setActionsFor(null); setPermanentDeletion(product); }} type="button">Eliminar definitivamente</button>}</div>}</div></div><p className="product-presentation"><strong>{presentationSummary(product.presentation)}</strong></p><dl className="product-macros"><div><dt>Kcal</dt><dd>{product.kilocaloriesPer100g}</dd></div><div><dt>Prot.</dt><dd>{decimal(product.proteinGramsPer100g)} g</dd></div><div><dt>Carbs.</dt><dd>{decimal(product.carbohydrateGramsPer100g)} g</dd></div><div><dt>Grasas</dt><dd>{decimal(product.fatGramsPer100g)} g</dd></div></dl></article>)}</div>}
